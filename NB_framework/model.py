...
if __name__ == "__main__":
    ...
    for i in range(len(neg_sents_test)):  # 用negative的句子做测试
        st = jieba.lcut(neg_sents_test[i])
        ...
        for word in st:
            if word in neg_dict_keys:
                p_neg += np.log((neg_dict[word] + 1) / (sum(neg_dict.values()) + len(neg_dict_keys)))
            else:
                p_neg += np.log(1 / (sum(neg_dict.values()) + len(neg_dict_keys)))

            if word in pos_dict_keys:
                p_pos += np.log((pos_dict[word] + 1) / (sum(pos_dict.values()) + len(pos_dict_keys)))
            else:
                p_pos += np.log(1 / (sum(pos_dict.values()) + len(pos_dict_keys)))
